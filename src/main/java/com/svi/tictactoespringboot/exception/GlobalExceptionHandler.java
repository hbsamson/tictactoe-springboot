...
public class GlobalExceptionHandler {
    ...
    ResponseEntity<ApiErrorResponse> malformed(HttpMessageNotReadableException e, HttpServletRequest r) { return response(HttpStatus.BAD_REQUEST,"MALFORMED_REQUEST","Request body is malformed",r,List.of()); }
    
    @ExceptionHandler(MethodArgumentTypeMismatchException.class)
    ResponseEntity<ApiErrorResponse> pathValue(MethodArgumentTypeMismatchException e, HttpServletRequest r) { return response(HttpStatus.BAD_REQUEST,"INVALID_PATH_VALUE","Invalid value for "+e.getName(),r,List.of()); }
    
    @ExceptionHandler(OptimisticLockingFailureException.class)
    ResponseEntity<ApiErrorResponse> concurrent(OptimisticLockingFailureException e, HttpServletRequest r) { return response(HttpStatus.CONFLICT,"CONCURRENT_MOVE","Game changed while the move was being processed",r,List.of()); }
    
    @ExceptionHandler(Exception.class)
    ResponseEntity<ApiErrorResponse> unexpected(Exception e, HttpServletRequest r) { 
        return response(HttpStatus.INTERNAL_SERVER_ERROR,"INTERNAL_ERROR","An unexpected error occurred", r, List.of()); 
    }
